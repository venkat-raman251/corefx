...
namespace System.Security.AccessControl
{
    internal static class Win32
    {
            ...
            out string resultSddl)
        {
            ...
            Interop.mincore.LocalFree(ByteArray);
        ...
        Error:
            ...
            if (errorCode == Interop.ERROR_NOT_ENOUGH_MEMORY)
            {
            ...
            )
        {
            ...
            try
            {
                ...
                if (errorCode == Interop.ERROR_SUCCESS && IntPtr.Zero.Equals(ByteArray))
                {
                    //
                ...
                else if (errorCode == Interop.ERROR_NOT_ALL_ASSIGNED ||
                         errorCode == Interop.ERROR_PRIVILEGE_NOT_HELD)
                {
                    throw new PrivilegeNotHeldException(Privilege.Security);
                }
                else if (errorCode == Interop.ERROR_ACCESS_DENIED ||
                    errorCode == Interop.ERROR_CANT_OPEN_ANONYMOUS)
                {
            ...
            GenericAcl dacl)
        {
            ...
            try
            {
                ...
                if (errorCode == Interop.ERROR_NOT_ALL_ASSIGNED ||
                    errorCode == Interop.ERROR_PRIVILEGE_NOT_HELD)
                {
                    throw new PrivilegeNotHeldException(Privilege.Security);
                }
                ...
                else if (errorCode != Interop.ERROR_SUCCESS)
                {
                    goto Error;
                }
            }
