        ...
        private static Luid LuidFromPrivilege(string privilege)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (false == Interop.mincore.LookupPrivilegeValue(null, privilege, ref luid))
                    {
                        int error = Marshal.GetLastWin32Error();

                        if (error == Interop.ERROR_NOT_ENOUGH_MEMORY)
                        {
                            throw new OutOfMemoryException();
                        }
                        else if (error == Interop.ERROR_ACCESS_DENIED)
                        {
                            throw new UnauthorizedAccessException();
                        }
                        else if (error == Interop.ERROR_NO_SUCH_PRIVILEGE)
                        {
        ...
        private sealed class TlsContents : IDisposable
        {
            ...
            public TlsContents()
            {
                ...
                if (error == Interop.ERROR_NOT_ENOUGH_MEMORY)
                {
                    ...
                    error == Interop.ERROR_CANT_OPEN_ANONYMOUS)
                {
                    throw new UnauthorizedAccessException();
                }
                else if (error != 0)
        ...
        private void ToggleState(bool enable)
        {
            ...
            finally
            {
                try
                {
                    ...
                    else if (Interop.ERROR_NOT_ALL_ASSIGNED == Marshal.GetLastWin32Error())
                    {
                        error = Interop.ERROR_NOT_ALL_ASSIGNED;
                    }
                    else
                    {
                        //
                        ...
                        this.initialState = ((previousState.Privileges[0].Attributes & Interop.SE_PRIVILEGE_ENABLED) != 0);
            ...
            if (error == Interop.ERROR_NOT_ALL_ASSIGNED)
            {
